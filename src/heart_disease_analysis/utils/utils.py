    ...
    with open(report_output_path, 'a') as file:
        ...
        for i, result in enumerate(results):
            ...
            if "classification_report" in result.keys():
                ...
                file.write(md_table)
                file.write(f"\n\n")
                file.write("* **Precision:** Of the cases predicted as a given class (0 or 1), the fraction that are actually that class (High precision = fewer false alarms).\n")
                file.write("  * Of patients classified as having heart disease, how many actually were?\n")
                file.write("* **Recall:** Of the cases that truly belong to a given class (0 or 1), the fraction the model correctly identifies  (High recall = fewer missed cases).\n")
                file.write("  * Of the people who actually had heart disease, how many were classified as such?\n")
                ...
                file.write("  * Overall percentage of correct guesses can be vulnerable to imbalances\n")
